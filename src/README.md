...
### ２．入庫・出庫処理による在庫数の変更と履歴データの整合性を保つ処理の実装しました。

### ３．Laravel APIとReact間で発生する認証エラーやバリデーションエラーを確認し、原因を切り分けながら調整した。

### ４． Laravel側の認証状態とReact側の画面遷移を連携させ、管理者・スタッフそれぞれの認証フローを整える点で苦労しました。
